  ...
  const navItems = [
    {
      ...
      title: "Dashboard",
      fields: ["Ecommerce", "Analytics"],
    },
    {
      icon: <FaUser size="1.5rem" />,
      title: "User",
      fields: ["Logout", "Forget Password", "User List"],
    },
    {
      icon: <FaBoxOpen size="1.5rem" />,
      title: "Product",
      fields: ["All Product", "Add Product"],
    },
    {
      icon: <RiFileList2Line size="1.5rem" />,
      title: "Invoice",
      fields: ["Ecommerce", "Analytics", "Crm"],
    },
  ];
  return (
    <div
      className={`space-y-2 transform transition-transform  min-w-[15rem] duration-700 ease-in-out max-sm:min-w-[10rem] bg-black ${
      ...
      {navItems.map((item, index) => (
        <div className="flex justify-center items-center flex-col w-full ">
          <div
            key={index}
            className={`${
              toggle == item?.title ? "bg-cyan-300" : ""
            } flex items-center gap-3 bg-gray-200 w-full p-2 rounded  hover:bg-gray-300 transition select-none hover:text-cyan-500 font-bold `}
            onClick={(e) => {
          ...
          <div
            className={`${
              toggle != item.title ? "hidden" : ""
            } w-full flex flex-col  pl-4 gap-2 transition-all duration-900 ease-in-out`}
          >
